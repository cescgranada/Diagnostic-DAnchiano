    ...
    <div className="flex flex-col gap-6 animate-in zoom-in duration-500">
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm print:hidden">
        <div className="flex items-center gap-4">
          ...
          <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Pàgina {currentPage + 1} de {pages.length}</span>
        </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden flex flex-col">
        <div className="bg-slate-50 px-8 py-4 border-b border-slate-200 flex justify-between items-center print:hidden">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold uppercase tracking-[0.2em] text-slate-500">Informe 360° - Secció {currentPage + 1}</span>
          </div>
          <button onClick={onReset} className="text-indigo-600 hover:text-indigo-800 text-sm font-bold flex items-center gap-2 transition-all">
        ...
        <div className="p-8 md:p-12">
          <div className="markdown-content prose prose-slate max-w-none">
            <div dangerouslySetInnerHTML={{ __html: transformMarkdown(pages[currentPage] || "") }} />
          </div>
        </div>

        {/* Botons de descàrrega integrats a la pàgina */}
        <div className="bg-slate-50 p-8 border-t border-slate-200 print:hidden">
          <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-6 text-center">Opcions d'exportació de l'informe complet</h4>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <button onClick={() => window.print()} className="flex flex-col items-center gap-2 bg-white p-4 rounded-2xl border border-slate-200 hover:border-indigo-400 hover:shadow-md transition-all group">
            ...
            <button onClick={exportMarkdown} className="flex flex-col items-center gap-2 bg-white p-4 rounded-2xl border border-slate-200 hover:border-indigo-400 hover:shadow-md transition-all group">
            ...
            <button onClick={exportPython} className="flex flex-col items-center gap-2 bg-white p-4 rounded-2xl border border-slate-200 hover:border-indigo-400 hover:shadow-md transition-all group">
            ...
            <button onClick={exportLaTeX} className="flex flex-col items-center gap-2 bg-white p-4 rounded-2xl border border-slate-200 hover:border-indigo-400 hover:shadow-md transition-all group">
  ...
  rows.slice(1).forEach(row => {
    ...
    if (cells.length > 0) {
      ...
      if (lowerRow.includes('fortaleses') || lowerRow.includes('oportunitats') || lowerRow.includes('debilitats') || lowerRow.includes('amenaces')) {
        rowClass = 'bg-slate-50 font-extrabold text-indigo-900';
      }
      ...
      html += `<tr class="${rowClass}">` + cells.map((c) => {
        ...
        // Suport per a llistes internes en cel·les (DAFO)
        let innerHTML = content;
        if (innerHTML.includes('- ')) {
          innerHTML = '<ul>' + innerHTML.split('- ').filter(i => i.trim()).map(i => `<li class="mb-2 list-disc ml-4">${i.replace(/\n/g, '<br/>')}</li>`).join('') + '</ul>';
        }
