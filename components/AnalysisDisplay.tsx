...
const AnalysisDisplay: React.FC<AnalysisDisplayProps> = ({ content, onReset }) => {
  ...
  const exportMarkdown = () => {
    downloadFile('informe-anchiano.md', content, 'text/markdown');
  };
...
\\end{document}`;
    downloadFile('informe-anchiano.tex', latex, 'application/x-tex');
  };
...
if __name__ == "__main__":
    print_report()
`;
    downloadFile('informe_anchiano.py', pythonCode, 'text/x-python');
  };
    ...
    <div className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden animate-in zoom-in duration-500">
      <div className="bg-slate-50 px-8 py-4 border-b border-slate-200 flex justify-between items-center print:hidden">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold uppercase tracking-widest text-slate-500">Informe Generat</span>
        </div>
        <button 
          onClick={onReset}
      ...
      <div className="bg-slate-50 px-8 py-10 border-t border-slate-200 print:hidden">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <button onClick={() => window.print()} className="flex items-center justify-center gap-3 bg-white border border-slate-200 p-4 rounded-2xl hover:border-indigo-300 transition-all">
            <span className="text-sm font-extrabold text-slate-700 text-center">Imprimir PDF</span>
          </button>
          <button onClick={exportMarkdown} className="flex items-center justify-center gap-3 bg-white border border-slate-200 p-4 rounded-2xl hover:border-indigo-300 transition-all">
            <span className="text-sm font-extrabold text-slate-700 text-center">Markdown (Word)</span>
          </button>
          <button onClick={exportLaTeX} className="flex items-center justify-center gap-3 bg-white border border-slate-200 p-4 rounded-2xl hover:border-indigo-300 transition-all">
            <span className="text-sm font-extrabold text-slate-700 text-center">LaTeX (Overleaf)</span>
          </button>
          <button onClick={exportPython} className="flex items-center justify-center gap-3 bg-white border border-slate-200 p-4 rounded-2xl hover:border-indigo-300 transition-all">
            <span className="text-sm font-extrabold text-slate-700 text-center">Python (Colab)</span>
          </button>
        </div>
      </div>
    </div>
...
function transformMarkdown(md: string): string {
  // Evitem doblar numeració eliminant el prefix si ja existeix una numeració al text
  let html = md
    ...
    // Numeració intel·ligent per no doblar 1. 1.1.
...
function renderTable(rows: string[]): string {
  ...
  rows.slice(1).forEach(row => {
    ...
    if (cells.length > 0) {
      ...
      const isSwot = lowerRow.includes('fortaleses') || lowerRow.includes('oportunitats') || lowerRow.includes('debilitats') || lowerRow.includes('amenaces');
      const rowClass = isSwot ? 'bg-slate-50 font-bold' : '';
