      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Grup (Ideal)</span>
          ...
          <textarea className="w-full h-32 p-3 text-sm border border-slate-200 rounded-xl bg-white focus:ring-2 focus:ring-indigo-500 transition-all resize-none shadow-sm" placeholder="Defineix les necessitats del context..." value={groupVal} onChange={(e) => groupSet(e.target.value)} />
          {groupFile && <p className="text-[10px] text-indigo-600 truncate max-w-[180px] font-medium">{groupFile.name}</p>}
        </div>
        {/* Comissió */}
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Comissió (Realitat)</span>
            <div className="flex items-center gap-1">
          ...
          <textarea className="w-full h-32 p-3 text-sm border border-slate-200 rounded-xl bg-white focus:ring-2 focus:ring-indigo-500 transition-all resize-none shadow-sm" placeholder="Defineix el perfil de la comissió..." value={commVal} onChange={(e) => commSet(e.target.value)} />
          {commFile && <p className="text-[10px] text-indigo-600 truncate max-w-[180px] font-medium">{commFile.name}</p>}
        </div>
      </div>
    </div>
  );
    ...
    <form onSubmit={handleSubmit} className="space-y-6 animate-in fade-in duration-500">
      <div className="bg-white border border-indigo-100 p-6 rounded-2xl shadow-sm flex gap-4 text-slate-800 items-center">
        ...
        <div className="text-sm">
          <p className="font-bold text-indigo-900 text-base mb-1">Anàlisi 360° D'Anchiano Blindat</p>
          <p className="text-slate-600 leading-relaxed">PROTOCOL: S'analitzen les 54 variables (18 per àmbit). Els resultats es divideixen en <strong>pàgines independents</strong> per garantir el rigor analític i facilitar la comparativa directa.</p>
        </div>
      </div>

      <div className="space-y-6">
        {renderSection("1. Competències (Saber Fer)", 
        ...
        {renderSection("2. Valors (Ètica i Propòsit)", 
        ...
        {renderSection("3. Personalitat (Saber Ser)", 
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
        <div className="flex flex-col sm:flex-row gap-6">
          <div className="flex-1 space-y-2">
            <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Motor d'Anàlisi</label>
            <select value={model} onChange={(e) => setModel(e.target.value)} className="w-full p-2 text-sm border border-slate-200 rounded-lg bg-white outline-none focus:border-indigo-500">
              <option value="gemini-3-flash-preview">Gemini 3 Flash (Recomanat)</option>
              <option value="gemini-3-pro-preview">Gemini 3 Pro (Màxima Precisió)</option>
            </select>
          </div>
          <div className="flex-1 space-y-2">
            <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Temperatura (Rigor vs Creativitat): {temperature}</label>
            <input type="range" min="0" max="1" step="0.1" value={temperature} onChange={(e) => setTemperature(parseFloat(e.target.value))} className="w-full accent-indigo-600 cursor-pointer" />
          </div>
        </div>

        <button type="submit" disabled={isLoading} className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-200 text-white font-extrabold py-4 px-8 rounded-2xl shadow-xl shadow-indigo-100 transition-all flex items-center justify-center gap-4 hover:scale-[1.01] active:scale-[0.99]">
          {isLoading ? (
            <svg className="animate-spin h-6 w-6 text-white" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
          ) : (
            <span className="text-lg uppercase tracking-wider">Generar Informe Blindat 360°</span>
          )}
        </button>
      </div>
    </form>
